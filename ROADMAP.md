...
> **Current state (2026-05-27): 6 chapters, 57 indexed terms, 99 static pages.** The authoritative live tracker is `CLAUDE.md` (Phase status + per-chapter detail + pick-up notes). Chapters added since session 2: Disability & Mental Health (14 terms, incl. the rejected-labels batch), Immigration & Citizenship (6), Class & Economic Status (4 — classism, ghetto, disadvantaged, working-class). The dated status blocks below are kept as historical session records.
...
- [~] **Disability & Mental Health** — **10 terms indexed** (disability, ableism, accessible, mental-health, survivor, victim, handicapped, mental-illness, addiction, deaf). Chapter intro with 6 cross-cutting principles. NCDJ dedicated source. `victim` is split-recommendation by context; `deaf` is the first specific-condition identity-first page. Candidates next: the rejected-label pages (addict, junkie, crazy, insane, r-word), specific conditions (autism/autistic 6, wheelchair 5, blind), injury (9), recovery, neurodiversity (4), suicide (5). `accessible` + `addiction` thinnest (3-4 srcs).
- [~] **Immigration & Citizenship** — **6 terms indexed** (immigrant, refugees, undocumented-immigrant, illegal-immigrant, illegal-alien, alien). Chapter intro with 6 cross-cutting principles. Define American + Immigrant Defense Project dedicated sources. Largely tapped for the current corpus. Next: a consolidated **Dreamer / DACA recipient** page (daca 3 + dream-act 3, but borderline scope + scrambled Define American OCR → expect PARTIAL). Below ≥3 bar (→ glossary long tail): anchor-baby (2), asylum-seeker (1), undocumented-worker (2). `noncitizen` attempted and dropped (only 1 real source). Deeper coverage (asylum-seeker, migrant, anchor-baby as full pages) needs a new discovered migration/asylum source.
- [ ] Age
