---
org: "National Center on Disability and Journalism"
org_slug: "ncdj"
work_title: "NCDJ Disability Language Style Guide"
year: 2021
copyright_holder: "National Center on Disability and Journalism (Arizona State University)"
license: "© NCDJ. Excerpted under fair use; original linked, not republished."
source_url: "https://ncdj.org/style-guide/"
local_archive: "source-guides/discovered/ncdj-disability-style-guide-2021.md"
host_posture: "private-mirror-link-out"
live_status: "live"
format: "markdown"
last_checked: 2026-05-18
added: 2026-05-17
---

## About

The **National Center on Disability and Journalism (NCDJ)** provides resources and guidance to journalists, content creators, and communicators who cover disability. It has been housed at the Walter Cronkite School of Journalism and Mass Communication at Arizona State University since 2008. The center states that it "does not advocate a particular point of view"; its concern is the journalistic principles of accuracy, fairness, and diversity in coverage of disability issues and of people with disabilities.

Its *Disability Language Style Guide* (2021) is the center's flagship reference — an alphabetical set of recommendations on disability-related terms, each with background and suggested usage. In the commons it anchors the Disability & Mental Health chapter, standing behind terms such as *disability*, *ableism*, *deaf*, and the chapter's rejected-label entries.

## Access

The guide is published openly on NCDJ's site (linked above) and was live as of the last check. Host posture is **private-mirror-link-out**: the commons keeps a private preservation copy for citation verification and links readers to NCDJ's authoritative version rather than republishing it. Quotes are held within fair-use limits.
