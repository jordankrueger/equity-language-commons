...
DSG, the most recent guide in this cluster (updated March 2024), treats Latine as **the more Spanish-usable gender-neutral form**, distinct from Latinx in structure even though the two share a function. TJA, the other recent source, registers the same direction more modestly: "some prefer Latine" to Latinx. The case for Latine is structural. The '-e' suffix functions as a gender-neutral marker in Spanish, where the '-x' suffix doesn't. Latine pronounces in Spanish (la-TEEN-eh); Latinx does not. Latine pluralizes cleanly (Latines), which DSG flags as one of its advantages; DSG gives Latinx the plural "Latinxs." Latine emerged inside Spanish-speaking LGBTQIA+ and feminist communities; Latinx is widely critiqued as an English-internet construction imposed on Spanish-speaking communities. None of this makes Latinx wrong, but Latine carries advantages wherever Spanish-language usability is a constraint.
