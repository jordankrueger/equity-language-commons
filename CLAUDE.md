...
2. **Codex ingestion slices DONE (2026-06-06, same session).** All 3 slices executed by Codex (driven by Claude, sandboxed worktree at `../elc-wt-source-discovery` — still up), verified + committed on branch `add-discovered-sources-2026-06` (pushed, NOT merged): 8 corpus sources ingested + extracted, UNHCR/IOM reference pages, pipeline rebuilt, build clean, lint = W6-only. **W6 report: `notes/source-discovery-2026-06-w6-report.md` — 17 newly-qualified terms** (D&MH 5: abnormal, cripple, differently-abled, functional-needs, limited-vision; S&GI 8: bathroom-bill, bigender, gender-identity-disorder, hermaphrodite, sex-change, sexual-preference, tranny, trans-woman; R&E 3: black-lives-matter, implicit-bias, reverse-racism; Faith 1: interfaith). Bonus: extract-pdfs.sh now takes explicit file targets (defuses global --force hazard).
2b. **NEXT TASK — finish the branch (Claude editorial work):**
   - **Add equity-focus framing to source pages (Jordan request 2026-06-06):** each new source's About section should say where it sits on the corpus spectrum — progressive equity guide (MSC, Opportunity Agenda, DCFPI, PICUM, HRW, 18Doors) vs identity-journalism/accuracy guide (Religion Stylebook, CAIR) vs legal-definitional reference (UNHCR, IOM, link-out-only). This posture taxonomy was the decision rule for the locked list (see research notes + 2026-06-06 session log) — make it visible to readers on the source pages.
   - Verify slice A/B extractions against live sources (Layer 1 conventions); polish all 10 source pages (About/Access prose, real metadata)
   - Graduate `jew`/`islam` from verified-hold (Religion Stylebook = 3rd source); remove from `notes/curated-glossary-overrides.yml`, write full pages
   - **Investigate why targeted terms didn't newly qualify:** poor/at-risk/vulnerable (Class), asylum-seeker/migrant (Migration), activist/tolerance/equality (Movement) are absent from W6 — check `notes/term-coverage-matrix.csv` directly; suspects: keyword-scan blocklist on common words, 1→2 source counts still under bar, existing aliases. Class & Movement chapter growth depends on this answer.
   - Write the 17 W6 term pages; re-run matrix + lint to strict-green; Layer 1/2 verification on new pages; merge → main → deploy
